...
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedAnt;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedAntOptimiser;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedAntOptimiser1;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedAntOptimiser2;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedAntOptimiser3;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedGraspMedoidsOptimiser;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedKMeansOptimiser;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedMemeticGAACOOptimiser;
import hu.u_szeged.inf.fog.simulator.workflow.aco.CentralisedTabuOptimiser;
...
public class WorkflowSimulation {
    ...
    public static void main(String[] args) throws Exception {
       ...
       // SeedSyncer.modifySeed(System.currentTimeMillis());
        ...
        WorkflowComputingAppliance node0 = new WorkflowComputingAppliance(cloudfile, "node0", new GeoLocation(48.8566, 2.3522), 0,false);  
        WorkflowComputingAppliance node1 = new WorkflowComputingAppliance(fogfile, "node1", new GeoLocation(51.5074, -0.1278), 0,true);  
        WorkflowComputingAppliance node3 = new WorkflowComputingAppliance(fogfile, "node3", new GeoLocation(41.9028, 12.4964), 0,true);  
        WorkflowComputingAppliance node5 = new WorkflowComputingAppliance(fogfile, "node5", new GeoLocation(43.7102, 7.2620), 0,true); 
        WorkflowComputingAppliance node6 = new WorkflowComputingAppliance(fogfile, "node6", new GeoLocation(55.6761, 12.5683), 0,true);  
        WorkflowComputingAppliance node8 = new WorkflowComputingAppliance(fogfile, "node8", new GeoLocation(48.2082, 16.3738), 0,true); 
        WorkflowComputingAppliance node9 = new WorkflowComputingAppliance(fogfile, "node9", new GeoLocation(50.8503, 4.3517), 0,true);  
        WorkflowComputingAppliance node10 = new WorkflowComputingAppliance(fogfile, "node10", new GeoLocation(46.7762, 23.6213), 0,true);  
        WorkflowComputingAppliance node11 = new WorkflowComputingAppliance(cloudfile, "node11", new GeoLocation(48.1351, 11.5820), 0,false);  
        WorkflowComputingAppliance node12 = new WorkflowComputingAppliance(fogfile, "node12", new GeoLocation(53.9076, 27.5754), 0,true);  
        WorkflowComputingAppliance node13 = new WorkflowComputingAppliance(fogfile, "node13", new GeoLocation(60.1695, 24.9354), 0,true); 
        WorkflowComputingAppliance node14 = new WorkflowComputingAppliance(fogfile, "node14", new GeoLocation(39.9334, 32.8597), 0,true);   
        WorkflowComputingAppliance node16 = new WorkflowComputingAppliance(fogfile, "node16", new GeoLocation(37.9838, 23.7275), 0,true);  
        WorkflowComputingAppliance node17 = new WorkflowComputingAppliance(fogfile, "node17", new GeoLocation(52.3702, 4.8952), 0,true);  
        WorkflowComputingAppliance node19 = new WorkflowComputingAppliance(cloudfile, "node19", new GeoLocation(51.1657, 10.4515), 0,false); 
      ...
      //  clusterAssignments = CentralisedAntOptimiser.runOptimiser(4, nodesToBeClustered, 100, 200, 0.50, 0.10, 0.10, 0.05);
        
     // (اختياري) لتثبيت النتائج:
      //  hu.mta.sztaki.lpds.cloud.simulator.util.SeedSyncer.centralRnd.setSeed(123456789L);
        SeedSyncer.centralRnd.setSeed(42L);
        System.out.println("Seed="+SeedSyncer.centralRnd.nextDouble());
        System.out.println("Seed="+SeedSyncer.centralRnd.nextDouble());
        SeedSyncer.centralRnd.setSeed(42L);
        System.out.println("Seed="+SeedSyncer.centralRnd.nextDouble());
    //    clusterAssignments =    CentralisedKMeansOptimiser.runOptimiserClusters(4,nodesToBeClustered,10,200,1e-4,1.0, 0.5, 1.0 );
        clusterAssignments = CentralisedMemeticGAACOOptimiser.runOptimiser(
                4, nodesToBeClustered,

                // GA (قوي للنشر)
                150, 800, 0.90, 0.06, 4, 3,

                // Memetic control
                20, 3,            // refineEveryR=20, refineEliteCount=3

                // ACO local refinement (صغير لأنه Local فقط)
                30, 50,           // ants=30, iters=50
                0.70, 0.20,       // probability=0.70, topPercent=0.20
                0.20, 0.15,       // pheromoneIncrement=0.20, evaporation=0.15
       ...
       // clusterAssignments = Medoids.runOptimiser(new ArrayList<>(List.of(4, 18, 15, 13)), nodesToBeClustered);
        ...
        System.out.println("clusterAssignments="+clusterAssignments);
        ...
        */
        ...
        for (int i = 0; i <clusterList.size() ; i++) { //clusterList.size()
        ...
        double totutilPM, totutilVM, singleVM;
        int pmCount=0, vmCount=0;double p = 0;
        for (int i = 0; i <clusterList.size() ; i++) {
        	totutilPM = 0; totutilVM = 0; singleVM=0;
        	pmCount=0; vmCount=0;
			for (WorkflowComputingAppliance pm : clusterList.get(i)) {
				for(VirtualMachine vm:pm.iaas.listVMs()) {
					if (vm.getPerTickProcessingPower()==0) continue;
					singleVM= (vm.getTotalProcessed())
						/ ((simuTimespan-WorkflowScheduler.realStartTime) * vm.getPerTickProcessingPower());
				totutilVM+=singleVM;
				vmCount++;
				//System.err.println("Cluster "+i+": "+pm.name+" "+vm.hashCode()+":    "+singleVM*100);
				 p=vm.getPerTickProcessingPower(); 
				}
			totutilPM += (pm.getLoadOfResource() )
					/ ((simuTimespan-WorkflowScheduler.realStartTime) * p);
			++pmCount;
		}
        
			 System.out.println("Cluster "+i+" Average utilisation of PMs: " + (100 * totutilPM) / pmCount + " %" +pmCount);
		     System.out.println("Cluster "+i+" Average utilisation of VMs: " + (100 * totutilVM) / vmCount + " %"+vmCount);
        }
